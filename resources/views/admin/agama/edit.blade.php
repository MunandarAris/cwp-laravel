        ...
        <div class="col-md-6">
            <div class="card card-primary">
                <div class="card-header">
                    <h3 class="card-title">Form Ubah Agama</h3>
                </div>
                <div class="card-body">
                    <form action="/agama24/{{$agama['id']}}" method="post" id="inputForm" enctype="multipart/form-data">
                        {{ csrf_field() }}
                        <input type="hidden" name="id" id="id" value="{{$agama['id']}}" />
                        <div class="form-group">
                            <label for="name">Agama</label>
                            <input type="text" id="name" name="name" class="form-control" placeholder="Nama Agama" value="{{old('name', $agama['name'])}}" required>
                            @error('name')
